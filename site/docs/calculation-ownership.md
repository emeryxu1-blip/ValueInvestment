...
#### Opportunity Range card data flow

The overview card is live-backed, not a browser-side fixture. The browser requests `GET /api/security/[exchange]/[symbol]/summary`; the route resolves the supported market code and the Worker calls AInvest through `lib/ainvest/client.ts` with server-only authentication. `lib/security/service.ts` normalizes the snapshot, parses the AInvest provider DCF value, obtains peer data when applicable, and returns the canonical `quote` and `valuation` fields. The browser formats those returned values and calculates only the visual range-marker positions in `ValuationRange`.

| Visible number | Source / calculation | Execution boundary |
| --- | --- | --- |
| Current price | AInvest snapshot quote in USD, exposed as `summary.quote.price.value` with `unit: "USD"` | Server / Worker fetches and normalizes; browser formats USD currency |
| Provider DCF value | Latest positive predicted price returned by AInvest `stockdiag_fundamental_value_dcf` (`fairValueModule`), in USD | Server / Worker parses provider data; browser formats USD currency |
| Earnings-power floor | `[min(positive TTM FCF, positive TTM net income) × 80% ÷ 10% + cash and marketable securities − debt] ÷ shares`; a no-growth screening cross-check, not a conventional forward DCF | Server / Worker (`calculateEarningsPowerFloor`); browser formats USD currency |
| Peer-based value | Median of valid positive P/E-, P/S-, and P/B-implied values from AInvest peer snapshots | Server / Worker (`getPeersResponse` and `derivePeerValue`) |
| Estimated value | `fairValue = dcfValue ?? peerValue`, exposed as `summary.valuation.fairValue.value` | Server / Worker (`getSecuritySummary`) |
| Implied value gap | `fairValue / current price - 1`, exposed as `summary.valuation.mispricing.value` | Server / Worker (`deriveMispricing`) |
| Range markers and labels | Positions are calculated from the returned price and method values; currency/percent strings are formatted for display | Browser only (`ValuationRange`, `money`, `signedPercent`) |

AInvest returns the supported US-equity monetary values in USD. The provider DCF value is a current AInvest reference, not a browser-generated estimate. The Worker filters invalid, nonfinite, zero, and negative predictions, chooses the latest remaining prediction, and preserves the value as unavailable when no supported prediction exists. No demo or locally generated substitute is inserted.

The overview chart plots the provider's positive dated DCF outputs as a
model-period curve. Those points come from one current provider snapshot and
can include a future period. They are not labelled as historical estimate
revisions. A genuine revision-history feature would require immutable,
point-in-time valuation snapshots with source dates and model versions.

| Calculation | Current authoritative code | Owner | Persist? | Migration status |
| --- | --- | --- | --- | --- |
| Provider DCF reference | AInvest `fairValueModule`, parsed in `lib/security/derivations.ts` | Provider-backed/backend canonical | No | Selects the latest positive finite provider prediction. Invalid and nonpositive predictions are ignored; missing output remains null. |
| No-growth earnings-power floor | `lib/security/intrinsic-value.ts` | Backend deterministic cross-check | No | Model-versioned and shown separately from selected fair value. Missing or nonpositive required inputs fail closed; null input timestamps remain null and are never replaced with quote/server time. |
| Reported and forecast FCF evidence | AInvest `stockdiag_fundamental_future_growthforecast`, parsed in `lib/security/valuation.ts` | Provider-backed/backend canonical | No | The API returns normalized points plus latest-four, forward-four, and forward-growth aggregates. Labels say “returned periods” because continuity and future-date status are not inferred. This evidence series is shown separately from the provider DCF value. |
...
| Summary fair-value selection | `lib/security/service.ts` | Backend canonical | No | Uses the provider DCF value when available and otherwise falls back to the peer estimate; the methods remain separately visible. |
...
| Calculation | Current authoritative code | Owner | Persist? | Migration status |
| --- | --- | --- | --- | --- |
| Provider DCF / price value gap | `lib/screener/service.ts`, using `deriveMispricing` | Backend canonical | Persist only as part of a durable screener snapshot | Uses the positive value at the latest returned provider DCF date divided by positive AInvest price, minus one. The stored value is published only after a complete server-side AInvest scan and validation; there is no Overview peer fallback in screener rows. |
...
| Provider-DCF-gap and positive P/E screens | `lib/screener/filter-presets.ts` | Backend canonical membership | Yes, in the immutable snapshot mask | The provider DCF value gap matches at 20% or more. The earnings-multiple screen requires a finite positive P/E no greater than 15×, so loss-making companies never pass through a negative multiple. |
...
| Filter-mask schema compatibility | `lib/screener/snapshot.ts` and `lib/screener/client-snapshot-contract.ts` | Backend and transport contract | Yes, on each generation | Schema-one and schema-two payloads remain readable during the schema-three rollout, while controls whose threshold semantics require schema three stay hidden until compatible stored data loads. Retired momentum and debt/equity bit positions remain reserved for old generations and are never written by schema three. Unknown future schemas fail closed. The public compact endpoint is `no-store` and revalidates the active D1 generation on every request. |
...
| Percent/currency display, value tone, and result caption | `components/screener/ScreenerClient.tsx` | Client presentation | No | The descriptive caption uses the unrounded displayed gap: above +1% positive, below -1% negative, and the inclusive interval between them within ±1%. It does not claim undervaluation relative to a provider DCF reference. |
